    ...
    <AppLayout>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-10">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-[10px] tracking-[0.4em] uppercase text-white/30">Third-Party Verification</span>
        </div>
        ...
        <p className="text-sm text-white/25 max-w-xl leading-relaxed">
          Verify that due diligence was completed without seeing the underlying financial data. Cryptographic proof only. Zero data leakage.
        </p>
      </motion.div>
      ...
      {verified && result && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="p-5 border border-white/[0.05]" style={{ background: 'rgba(255,255,255,0.005)' }}>
              ...
              {Object.entries(result).map(([k, v]) => (
                <div key={k} className="flex items-center justify-between py-1.5 border-b border-white/[0.02]">
                  <span className="text-[10px] text-white/12">{k}</span>
                  <span className="text-[10px] font-mono text-white/20 truncate max-w-[180px]">{v}</span>
                </div>
              ))}
            </div>
            <div className="p-5 border border-white/[0.05]" style={{ background: 'rgba(255,255,255,0.005)' }}>
              <div className="text-[9px] tracking-[0.15em] uppercase text-white/18 mb-4">Claims</div>
              {[['DueDiligenceCompleted', 'true'], ['AnalysisDigest', '—'], ['Scope', '—'], ['Compliance', '—']].map(([k, v]) => (
                <div key={k} className="flex items-center justify-between py-1.5 border-b border-white/[0.02]">
                  <span className="text-[10px] text-white/12">{k}</span>
          ...
          <div className="flex items-start gap-4 mb-8 p-5 border border-white/[0.04]" style={{ background: 'rgba(255,255,255,0.003)' }}>
            <div>
              <div className="text-sm font-semibold text-white/20 mb-1">What This Verification Proves</div>
              <div className="text-[10px] text-white/10 leading-relaxed">
                This credential proves due diligence was completed to cryptographic standards. It does NOT expose underlying financial data, cap table, or any PII that was analyzed. All data processing occurred inside Intel TDX TEE with immutable audit trail.
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <button onClick={() => { navigator.clipboard.writeText(did); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
              ...
              <Copy className="w-3.5 h-3.5" />{copied ? 'Copied' : 'Copy Proof'}
            </button>
      ...
      {!verified && !verifying && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          className="p-12 text-center border border-white/[0.03]" style={{ background: 'rgba(255,255,255,0.003)' }}>
          <div className="text-sm text-white/12 mb-1">Enter a did:t3n to verify due diligence credentials</div>
          <div className="text-[10px] text-white/6">No underlying data is exposed. Only cryptographic proof of completion.</div>
        </motion.div>
      )}
    </AppLayout>
  )
