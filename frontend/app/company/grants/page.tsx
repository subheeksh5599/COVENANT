...
export default function CompanyGrantsPage() {
  const [grants, setGrants] = useState<{ id: string; grantee: string; functions: string[]; hosts: string[]; active: boolean; callCount: number; maxCalls: number }[]>([])
    ...
    <AppLayout tabs={tabs}>
      ...
      {grants.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          <div className="text-sm text-white/15 mb-2">No active grants</div>
          <div className="text-[10px] text-white/8 max-w-md mx-auto">
            Seal documents first, then create an agent-auth-update grant to give an investor agent scoped access to specific contracts and time windows.
          </div>
        </motion.div>
      ) : (
          ...
          {grants.map((grant, i) => (
            <motion.div key={grant.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
                ...
                <div><div className="text-[8px] tracking-[0.15em] uppercase text-white/15 mb-1">Call Limit</div><div className="text-[10px] font-mono text-white/35">{grant.callCount}/{grant.maxCalls}</div></div>
              </div>
